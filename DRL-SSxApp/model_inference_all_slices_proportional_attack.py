...
def run_inference_epoch(agent, num_episodes, malicious_chance):
    ...
    # what each slice will get:
    mal_per_slice = num_malicious_episodes // 3
    # extra leftover
    extra_mal = num_malicious_episodes % 3

    malicious_slice_idx = [0] * mal_per_slice + [1] * mal_per_slice + [2] * mal_per_slice
    malicious_slice_idx.extend(random.sample([0,1,2], extra_mal))

    random.shuffle(malicious_slice_idx)
    ...
    for episode in range(num_episodes):
        ...
        if episode in malicious_episodes:
            slice_idx = malicious_slice_idx.pop()
            DL_BYTE_TO_PRB_RATES[slice_idx] *= 10
            is_mal[slice_idx] = True
        state = [
        ...
        for i in range(3):
            ...
            elif not is_mal[i]:  # Slice isnt malicious
                episode_throughput += state[i]
                slice_throughput[i] += state[i]
            else:
    ...
    return accuracy, avg_throughput, avg_slice_throughput
